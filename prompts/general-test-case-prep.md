Look into the library `/home/pings/projects/cangjie-resumptions/cangjie_runtime/stdlib/libs/std/io`. 
...
Ensure you cover as much of the public functions of `io` as possible. 
...
- `io_testing_entry.cj` - contains the entry point: it should contain the "dummyTester" function, defined as in the existing examples, and run the fuzzer in a loop for 1000 iterations. 
- `io_testing_generators.cj` - contains the actual random generator and calls to the libraries.
- `io_testing_lib.cj` - contains the defined executors: the function along with their generators.

The success criterion for a fuzzer is that running the fuzzer in a loop for 1000 iterations should pass without errors.

If you believe a genuine bug is found within the `io` library, in which case these should be recorded down in an .md file separately, then the bug trigger is to be skipped temporarily.

Let the agent know that the script `./cjpm-run.sh` can be used to run the function `main.cj`. A function call is alreayd present `main_io_testing()`: this is to be the entry point of the `io` fuzzer.

Please provide a plan for an agent to follow, by examining the `io` library.
Decide what information does the context need to include, what generators are to be used for each function in the `io` library.
